...
print("appen 전후 같은 list인가?", before_id == id(lst))

# list.insert(index, 값)--------------------------------
# - index에 값을 삽입하는 메서드
# - 지정된 index 부터 뒤에 있는 모든 list 값의 index가 1씩 중가(밀려남)
print('--- list.insert() ---')
lst.insert(1, 1.5)
lst.insert(0, 0)
...
lst.pop(2)
print("lst :", lst)
print("변경 후 id", before_id == id(lst))
...
print(len(students[0][0])) # 3(홍길동 글지 수)
...
print('커스텀 정렬 기준 함수 :',fruits)


# sorted() : 원본 유지 정렬 (새 리스트를 반환)
print('--- sorted(list) ---')
nums = [9, 2, 4, 7, 1]
nums2 = sorted(nums)
print("원본 nums: ", nums)
print("정렬된 nums2: ", nums2) # 오름차순 정렬
...
# d = 0번 인덱스 요소 (10)
# *e = 나머지 1, 2번 인덱스 요소 [20,30] -> 나머지를 list 형태로 반환
d, *e = numbers
print("d :",d,", e :", e)
